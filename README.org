...
TODO tab should be a session list that when selecting a session saves currentone before switch (maybe telescope) and the dashboard should be in D
|-------+----------+----------------|
